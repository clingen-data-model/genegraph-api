(ns genegraph.api.graphql.schema.find
  (:require [genegraph.framework.storage.rdf :as rdf :refer [create-query]]
            [clojure.string :as s]))

;; TODO this one will require some testing. At least it compiles...
;; also we aren't currently building text indexes.
...
(defn query [_ args _]
  (let [limit-offset-sort-params (-> args
        ...
        query-params (-> (if (string? (:text args))
                         ...
                         (assoc ::rdf/params limit-offset-sort-params))
        query (if (:text args)
                query-with-text-search
                query-without-text-search)
        result-count (future (query (assoc query-params ::rdf/params {:type :count})))]
